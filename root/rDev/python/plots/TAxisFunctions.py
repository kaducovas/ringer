...
def AutoFixAxes(can,ignoreErrors=False, both=False, ymaxFrac=1.0) :
    ...
    SetYaxisRanges(can,miny,maxy*ymaxFrac)
    return
...
def MinimumForLog(can) :
    from ROOT import TGraph,TH1,TMath,TEfficiency
    ymin = 999999999
    for i in can.GetListOfPrimitives() :
        ...
        if issubclass(type(i),TH1) :
            for bin in range(i.GetNbinsX()) :
                ...
                if y <= 0 :
                    y = ymin
                ymin = min(ymin,y)
        if issubclass(type(i),TEfficiency) :
...
def SetYaxisRanges(can,ymin,ymax) :
    ...
    from ROOT import TGraph,TH1,TEfficiency
    yaxis = 0
    for i in can.GetListOfPrimitives() :
        if issubclass(type(i),TGraph) :
        ...
        if issubclass(type(i),TH1) :
            ...
            break
        if issubclass(type(i),TEfficiency) :
            yaxis = i.GetPaintedGraph().GetYaxis()
            break
    if not yaxis :
...
def SetXaxisRanges(can,xmin,xmax) :
    ...
    for i in can.GetListOfPrimitives() :
        ...
        if issubclass(type(i),TH1) :
            ...
            break
        if issubclass(type(i),TEfficiency) :
            xaxis = i.GetPaintedGraph().GetXaxis()
            break
    if not xaxis :
